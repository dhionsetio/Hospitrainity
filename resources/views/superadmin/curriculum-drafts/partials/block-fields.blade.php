...
<label class="block text-sm font-semibold">{{ __('admin.attached_asset_optional') }}<select name="asset_public_id" class="mt-1 block w-full rounded-md border-neutral-300"><option value="">{{ __('admin.no_attached_asset') }}</option>@foreach($assets as $asset)<option value="{{ $asset->public_id }}" @selected(($selectedAssetId ?? null) === $asset->public_id)>{{ $asset->display_name }} ({{ __('admin.asset_kinds.'.$asset->kind->value) }})</option>@endforeach</select></label>
