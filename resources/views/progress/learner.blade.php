                ...
                @else
                    <ul class="mt-3 flex flex-wrap gap-2">
                        @foreach($detail['inventory'] as $version)
                            ...
                            <li>
                                <a href="{{ route($detailRouteName, ['learner' => $detail['learner'], 'package' => $version['package_name'], 'version' => $version['content_version']]) }}"
            ...
            <div class="mt-6 space-y-6">
                @forelse($detail['hierarchy'] as $chapter)
                    <section class="rounded-lg bg-white p-6 shadow-sm" aria-labelledby="chapter-{{ $chapter['code'] }}">
                        <div class="border-b border-neutral-200 pb-4">
                            ...
                            <p class="mt-1 text-xs text-neutral-500">{{ $chapter['code'] }} <span aria-hidden="true">·</span> {{ $chapter['lifecycle_status'] }}</p>
                        </div>
                        <div class="mt-5 space-y-6">
                            @forelse($chapter['sections'] as $section)
                                <section aria-labelledby="section-{{ $section['code'] }}">
                                    ...
                                    <p class="mt-1 text-xs text-neutral-500">{{ $section['code'] }}</p>
                                            ...
                                            <table class="w-full text-left text-sm text-neutral-700">
                                                ...
                                                <thead class="bg-neutral-50 text-xs uppercase text-neutral-600">
                                                    <tr>
                                                        ...
                                                        <th scope="col" class="px-4 py-3">{{ __('admin.notes') }}</th>
                                                    </tr>
                                                </thead>
                                                <tbody class="divide-y divide-neutral-200">
                                                    ...
                                                    <tr>
                                                        <th scope="row" class="px-4 py-3 font-semibold text-neutral-900">
                                                            {{ $activity['title'] }}
                                                            <span class="mt-1 block font-mono text-xs font-normal text-neutral-500">{{ $activity['code'] }}</span>
                                                        </th>
                ...
                @empty
                    @if($detail['selection']['content_version'] === null)
                        <div class="rounded-lg bg-white p-8 text-center text-neutral-600 shadow-sm">{{ __('admin.no_progress_or_packages') }}</div>
                    @elseif($detail['unresolved']->isEmpty())
                        <div class="rounded-lg bg-white p-8 text-center text-neutral-600 shadow-sm">{{ __('admin.no_canonical_hierarchy_for_version') }}</div>
                    @endif
                @endforelse

                @if($detail['unresolved']->isNotEmpty())
