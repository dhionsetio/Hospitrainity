---
name: impeccable
description: Use automatically for evaluating or incrementally improving an existing frontend through critique, audit, polish, clarification, hardening, optimization, responsive adaptation, animation, color, design-system extraction, or live iteration. Prefer frontend-design for greenfield UI unless the user explicitly invokes Impeccable init, craft, or shape. Do not use for backend-only work, pure browser testing, or standalone prose editing.
version: 3.9.1
---
...
Resolve the directory containing this `SKILL.md` as `<skill-dir>`. Every command in this skill or its references that names `.agents/skills/impeccable` must use `<skill-dir>` instead when the skill is loaded from a user-level location. Always keep the command working directory at the user's active project.
