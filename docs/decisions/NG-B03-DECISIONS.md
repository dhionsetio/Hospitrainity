...
## 2026-07-20 privileged-navigation correction

An owner-reported browser reproduction confirmed that the initial implementation applied the privileged MFA middleware to every authenticated web request. That placement protected privileged work, but it also redirected safe public, help, context-selection, and Learner surfaces to the security center, creating an unintended navigation trap.

The accepted MFA requirement is unchanged. MFA is now enforced explicitly on the Supervisor/Instructor, Content Admin, and System Admin route groups. An unenrolled privileged account may visit public and self-service surfaces or deliberately continue in Learner mode, where privileged routes remain unavailable. The security center explains this boundary and offers explicit exits to Learner mode and the public home page. No authenticator, recovery code, session, account, or authoritative record was removed or bypassed.
